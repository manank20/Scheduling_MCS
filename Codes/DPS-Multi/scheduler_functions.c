...
void find_hyperperiods(task_set_struct *task_set, processor_struct *processor, double *hyperperiod)
{
    ...
    for (num_core = 0; num_core < processor->total_cores; num_core++)
    {
        ...
        {
            if (tasks_list[num_task].core == num_core)
            {
                first_task = num_task;
                break;
            }
        }
        lcm = tasks_list[first_task].period;
    ...
    for (i = 0; i < processor->total_cores; i++)
    {
        ...
        if (processor->cores[i].completed_scheduling == 0)
        {
            ...
            if (processor->cores[i].curr_exec_job != NULL)
            {
                ...
                if (processor->cores[i].curr_crit_level == LOW)
                {
...
void remove_jobs_from_queue(job_queue_struct **ready_queue, task *tasks_list)
{
    ...
    while (temp && temp->next)
    {
        ...
        {
            temp->absolute_deadline -= tasks_list[temp->task_number].virtual_deadline;
            temp->absolute_deadline += tasks_list[temp->task_number].relative_deadline;
            temp = temp->next;
        }
    }
...
*/
void find_job_parameters(task *tasks_list, job *new_job, int task_number, int job_number, double release_time, int curr_crit_level)
{
    double actual_exec_time;

    new_job->release_time = release_time;
    actual_exec_time = find_actual_execution_time(tasks_list[task_number].WCET[curr_crit_level], tasks_list[task_number].criticality_lvl);

    if (tasks_list[task_number].criticality_lvl == LOW)
    {
        new_job->execution_time = actual_exec_time;
    }
    else
    {
        new_job->execution_time = min(actual_exec_time, tasks_list[task_number].WCET[HIGH]);
    }
...
void update_job_arrivals(job_queue_struct **ready_queue, task_set_struct *task_set, int curr_crit_level, double arrival_time, int core_no)
{
    ...
    {
        if (tasks_list[curr_task].core == core_no)
        {
            ...
            if (release_time <= arrival_time && criticality_lvl >= curr_crit_level)
            {
                ...
                // fprintf(output_file, "Job %d, %d arrived\n", curr_task, tasks_list[curr_task].job_number);
                new_job = (job *)malloc(sizeof(job));
                find_job_parameters(tasks_list, new_job, curr_task, tasks_list[curr_task].job_number, release_time, curr_crit_level);
...
void schedule_taskset(task_set_struct *task_set, processor_struct *processor, FILE *output_file)
{
    ...
    while (1)
    {
        ...
        else if (decision_point == CRIT_CHANGE)
        {
            ...
            for (num_core = 0; num_core < processor->total_cores; num_core++)
            {
                remove_jobs_from_queue(&processor->cores[num_core].ready_queue, tasks_list);
                reset_virtual_deadlines(&task_set, num_core);
