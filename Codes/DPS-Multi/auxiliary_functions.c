...
*/
double find_actual_execution_time(double exec_time, int crit_lvl)
{
    double n = rand() % 3;
    if (crit_lvl == LOW)
    {
...
void set_virtual_deadlines(task_set_struct **task_set, int core_no, double x)
{
    int num_task;
...
void reset_virtual_deadlines(task_set_struct **task_set, int num_core)
{
    int i;
    for (i = 0; i < (*task_set)->total_tasks; i++)
    {
        if ((*task_set)->task_list[i].criticality_lvl == HIGH && (*task_set)->task_list[i].core == num_core)
