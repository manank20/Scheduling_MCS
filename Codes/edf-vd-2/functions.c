...
decision_struct find_decision_point(task_set_struct* task_set, job* ready_queue, kernel_struct* kernel) {
    ...
    int decision_time = min(min(arrival_time, completion_time), WCET_counter);
...
*/
void find_job_parameters(task* tasks_list, job* new_job, int task_number, int release_time, int curr_crit_level){
...
void schedule_new_job(kernel_struct *kernel, job_queue_struct* job_queue, task_set_struct* task_set) {
    ...
    job_queue->job_list_head->scheduled_time = kernel->total_exec_time; 
    job_queue->job_list_head->completed_job_time = kernel->total_exec_time + (job_queue->job_list_head->execution_time - job_queue->job_list_head->actual_execution_time); 
...
void schedule_taskset(task_set_struct* task_set, kernel_struct* kernel, FILE* output_file){
    ...
    while(kernel->total_exec_time <= hyperperiod) {
        ...
        int decision_time = decision.decision_time;
        ...
        if(decision_time == hyperperiod) {
            kernel->total_idle_time += (decision_time - kernel->total_exec_time);
            kernel->total_exec_time = decision_time;
        ...
        if(decision_point == ARRIVAL) {
            ...
            if(kernel->curr_exec_job == NULL) {
                kernel->total_idle_time += (decision_time - total_exec_time);
