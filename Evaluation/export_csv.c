...
int main() {
    ...
    for(int i=0; i<5; i++)
    {
        ...
        char dirname[100] = "./discarded/util_4/sample_taskset_";
        char ext[2];
        sprintf(ext, "%d", i+1);
        strcat(dirname, ext);
    ...
    for(int i=0; i<4; i++)
    {
        ...
        static_energy[i] += (total_at[i] + total_it[i]) * 0.5 / total_at[i];
        ...
        fprintf(cores, "%.2lf %.2lf %.2lf\n", total_at[i], total_it[i], total_st[i]);
    }
